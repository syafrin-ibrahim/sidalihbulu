...
switch($act){
	default :
			echo"<div class='row'>
                <div class='col-lg-12'>
                    <h1 class='page-header'>Data Pemilih Pilolaheya</h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
			<button type='button' class='btn' onclick=window.location.href='?syafrin=P_pilolaheya&act=tambah'>input data pemilih</button>
			<button type='button' class='btn' onclick=window.location.href='?syafrin=P_pilolaheya&act=saring'>pemilih tersaring</button>
			<button type='button' class='btn' onclick=window.location.href='?syafrin=P_pilolaheya&act=gandanik'>pemilih ganda nik</button>
			<button type='button' class='btn' onclick=window.location.href='?syafrin=P_pilolaheya&act=gandanama'>pemilih nama sama</button>
			<br/><br/>
			<div class='row'>
            <div class='col-lg-12'>
			<div class='panel panel-default'>
				<div class='panel-heading'>";
                 $fg=mysqli_query($konek, "select * from pemilih where ID_DESA=5 and saring=0");
				$jml=mysqli_num_rows($fg);
				echo" Jumlah Pemilih Desa Pilolaheya &nbsp; &nbsp;<strong> $jml</strong>";                       
                echo"</div>";
			...
			<table class='table table-striped table-bordered table-hover' id='dataTables-example'>
				<thead>
					...
					$no=1;
			$query="select * from pemilih where ID_DESA=5 and SARING=0 order by NKK asc";
			...
			while($data=mysqli_fetch_array($tampilku)){
				...
				<td>".$data['NAMA']."</td>
				<td>".$data['TPS']."</td>
				<td align='center'><a href=?syafrin=P_pilolaheya&act=penyaringan&id=".$data['DP_ID'].">saring</i></a></td>
				<td align='center'><a href=?syafrin=P_pilolaheya&act=edit&id=".$data['DP_ID']."><i class='fa fa-edit'></i></a></td>
				<td align='center'><a href='$go?syafrin=P_pilolaheya&perintah=hapus&id=".$data['DP_ID']."' onclick=\"return confirm('yakin akan menghapus data ini ?')\">
			...
			echo"
				...
				<div class='panel panel-default'>
						...
						<div class='col-lg-6'>
								...
								<form  role='form' class='form-group' method='post' action='$go?syafrin=P_pilolaheya&perintah=simpan'>
...
break;
case 'edit' :
				$er=mysqli_query($konek, "select * from pemilih where ID_DESA=5 and DP_ID='".$_GET['id']."'");
			...
			echo"
				...
				<div class='panel panel-default'>
						...
						<div class='col-lg-6'>
								...
								<form  role='form' class='form-group' method='post' action='$go?syafrin=P_pilolaheya&perintah=update'>
...
case 'saring' : 
		echo"<div class='row'>
			...
			<button type='button' class='btn' onclick=window.location.href='?syafrin=P_pilolaheya'>lihat pemilih aktif</button>
			...
			<table class='table table-striped table-bordered table-hover' id='dataTables-example'>
				<thead>
					...
					$no=1;
			$query="select * from pemilih where ID_DESA=5 and SARING!=0";
			$tampilku=mysqli_query($konek, $query);
			$tanda="";
			while($data=mysqli_fetch_array($tampilku)){
			if($data['SARING'] == 1){
				...
				<td align='center'><a href='$go?syafrin=P_pilolaheya&perintah=kembalikan&id=".$data['DP_ID']."' onclick=\"return confirm('yakin akan mengembalikan data ini ?')\">
				kembalikan</a></td>
		...
		$er=mysqli_query($konek, "select * from pemilih where ID_DESA=5 and DP_ID='".$_GET['id']."'");
			...
			echo"
				...
				<div class='panel panel-default'>
						...
						<div class='row'>
						<div class='col-lg-6'>
						<form  role='form' class='form-group' method='post' action='$go?syafrin=P_pilolaheya&perintah=penyaringan'>
...
case 'gandanik' :
			...
			<table class='table table-striped table-bordered table-hover' id='dataTables-example'>
				<thead>
					...
					$no=1;
			$query="SELECT NIK, NAMA, COUNT(*) as JML from pemilih where ID_DESA=5 and NIK!=0 and SARING=0 group by NIK HAVING COUNT(NIK) > 1";//"select * from pemilih where ID_DESA=5 and SARING=0";
			$tampilku=mysqli_query($konek, $query);
